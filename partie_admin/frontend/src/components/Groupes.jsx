...
export default function Groupes() {
  ...
  const [activeNav, setActiveNav] = useState("Groupes");

  const apiUrl = "http://localhost:8084/api/groupes";
  const niveauUrl = "http://localhost:8084/niveaux";
  const specUrl = "http://localhost:8084/specialites";

  // Fonction pour recharger les groupes (retry)
  const reloadGroupes = () => {
    setError(null);
    loadGroupes();
  };
  ...
  const loadGroupes = () => {
    loadEndpoint(apiUrl, setGroupes, "groupes");
  };
  ...
  useEffect(() => {
    loadEndpoint(niveauUrl, setNiveaux, "niveaux");
  }, []);

  useEffect(() => {
    loadEndpoint(specUrl, setSpecialites, "spécialités");
  }, []);

  // vérification locale des doublons (sur nom + numero + niveau)
  const isDuplicateLocal = (nomValue, numeroValue, niveauValue, excludingId = null) => {
    const n = (nomValue || "").trim().toLowerCase();
    const num = parseInt(numeroValue || "0");
    return groupes.some(gr => {
      if (excludingId && gr.id === excludingId) return false;
      const gn = (gr.nom || "").trim().toLowerCase();
      const gnum = gr.numero || 0;
      const gniv = gr.niveau?.id;
      return gn === n && gnum === num && gniv === niveauValue;
    });
  };
  ...
  const isFormValid = () => {
    return nom.trim() && numero.trim() && niveauId && specialiteId;
  };

  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!isFormValid()) {
      alert("Veuillez remplir le nom, le numéro, sélectionner un niveau et une spécialité.");
      return;
    }
    // Vérif locale avant envoi
    if (isDuplicateLocal(nom, numero, parseInt(niveauId), editId)) {
      alert("Ce nom et numéro existent déjà pour ce niveau. Merci de modifier.");
      return;
    }
    setIsLoading(true);
    setError(null);
    ...
    try {
      ...
      const errorMsg = "Impossible d'enregistrer le groupe : " + err.message;
      setError(errorMsg);
      alert(errorMsg);
    } finally {
      setIsLoading(false);
  ...
  const handleEdit = (g) => {
    ...
    setNiveauId(g.niveau?.id?.toString() || "");
    setSpecialiteId(g.specialite?.id?.toString() || "");
    setEditId(g.id);
    setIsModalOpen(true);
  };
        ...
        <div className="min-h-screen bg-gradient-to-br from-gray-50 to-blue-50">
            ...
            <div className="max-w-7xl mx-auto px-6 py-4">
              <div className="flex justify-between items-center">
                <div className="flex items-center gap-3">
                  ...
                  <h1 className="text-xl font-bold text-gray-900">Gestion Groupes</h1>
                </div>
              </div>
            </div>
          </header>
          <div className="max-w-7xl mx-auto px-6 py-8">
            ...
            <div className="flex justify-between items-center mb-8">
              <div>
                <h2 className="text-3xl font-bold text-gray-900 mb-1">Groupes</h2>
                <p className="text-gray-600">
                  Total: <span className="font-semibold text-blue-600">{groupes.length}</span> groupe{groupes.length > 1 ? 's' : ''}
                </p>
              </div>
              <button
                onClick={openModal}
                ...
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-gradient-to-r from-gray-50 to-blue-50">
                    <tr>
                      ...
                      <th className="px-6 py-4 text-left text-xs font-bold text-gray-600 uppercase tracking-wide">Numéro</th>
                      ...
                      <tr>
                        <td colSpan="6" className="px-6 py-20 text-center">
                          <div className="flex flex-col items-center">
                            ...
                            <h3 className="text-lg font-bold text-gray-900 mb-2">Aucun groupe</h3>
                            <p className="text-sm text-gray-500 mb-6">Commencez par créer votre premier groupe</p>
                            <button
                              onClick={openModal}
                      ...
                      groupes.map((g) => (
                        <tr key={g.id} className="hover:bg-blue-50/50 transition-colors group">
                          <td className="px-6 py-4 whitespace-nowrap">
                            <span className="text-sm font-bold text-gray-900">{g.nom}</span>
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap">
                            <span className="text-sm font-medium text-gray-700">{g.numero || 'N/A'}</span>
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h3 className="text-lg font-bold text-gray-900">
                          {editId ? "Modifier" : "Créer un groupe"}
                        </h3>
                        <p className="text-xs text-gray-500">
                          {editId ? "Modifier les informations" : "Ajouter un nouveau groupe"}
                        </p>
                      </div>
                    </div>
                    <button
